...
UploadManager.prototype = {
	...
	addFile: function(file, fileNumTotal, postHeaders, stream, fileDone) {
		var enc = new ArticleEncoder(file.name, file.size, this.articleSize, this.opts.bytesPerLine, this.dateOverride, this.opts.articleEncoding);
		var sizes = [];
		var self = this;
				...
				if(nzb) {
					if(!nzbFile) {
						var nzbArgs = [
							...
							{subject: postHeaders.subject, file: file.file},
							postHeaders.from,
